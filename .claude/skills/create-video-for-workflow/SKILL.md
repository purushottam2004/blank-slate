...
1. Put the temporary spec in a `video_test/` directory beside the suite it belongs to.
   Examples:
   - `e2e/tests/video_test/login-hello.spec.ts`
   Fix relative imports for the extra directory (`../helpers/auth`).
2. Reuse existing helpers (`e2e/tests/helpers/auth.ts`) when the flow needs login or seed data.
3. Record with video enabled. Agent sandboxes (Claude Code, Cursor, and similar) may set
   `PLAYWRIGHT_BROWSERS_PATH` to an empty cache under `/tmp`.
...
PLAYWRIGHT_BROWSERS_PATH="$USER_PW" E2E_VIDEO=1 E2E_IMAGES=1 npx playwright test --headed --workers=1 --project=web path/to/your.spec.ts
```
...
Or the project scripts: `npm run test:web:headed:video` / `npm run test:headed:video:images` when they match (still prefix `PLAYWRIGHT_BROWSERS_PATH="$HOME/.cache/ms-playwright"` in agent runs).
